<template>
	<view class="user-chat-item animated fadeIn fast">
		<block v-for="(item,index) in getNewsContent" :key="index">
			<view class="user-chat-list u-f" :class="{'user-chat-me':item.id == myId}">
				<!---->
				<u-image class="pic"  v-if="item.id != myId" width="80rpx"  shape="circle" :src="`${baseUrl}${toImg}`" mode="widthFix"></u-image>
				...
				<!--是否为自己-->
				<u-image class="pic" v-if="item.id == myId"  width="80rpx" shape="circle"  :src="`${baseUrl}${myImg}`" mode="widthFix" ></u-image>
			</view>
		</block>
	</view>
</template>
